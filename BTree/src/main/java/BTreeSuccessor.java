...
/**
 * B-tree implementation that uses successor instead of predecessor for internal node removal.
 * Based on the original BTree implementation but modified to use the smallest value from
 * the right subtree when removing from internal nodes.
 * 
 * @author Modified from Justin Wetherell's implementation
 */
@SuppressWarnings("unchecked")
public class BTreeSuccessor<T extends Comparable<T>> implements ITree<T> {
     ...
     *            of the B-Tree.
     */
    public BTreeSuccessor(int order) {
    ...
    private T remove(T value, Node<T> node) {
        ...
        if (node.numberOfChildren() == 0) {
            ...
            // internal node - MODIFIED TO USE SUCCESSOR INSTEAD OF PREDECESSOR
            // Instead of using the greatest from left subtree (predecessor),
            // we use the smallest from right subtree (successor)
    ...
    /**
     * Remove smallest valued key from node.
     * 
     * @param node
     *            to remove smallest value from.
    ...
    public java.util.Collection<T> toCollection() {
        return (new JavaCompatibleBTreeSuccessor<T>(this));
    }
    ...
    private static class TreePrinter {
        ...
        public static <T extends Comparable<T>> String getString(BTreeSuccessor<T> tree) {
    ...
    public static class JavaCompatibleBTreeSuccessor<T extends Comparable<T>> extends java.util.AbstractCollection<T> {

        private BTreeSuccessor<T> tree = null;

        public JavaCompatibleBTreeSuccessor(BTreeSuccessor<T> tree) {
            this.tree = tree;
        }
        ...
        public java.util.Iterator<T> iterator() {
            return (new BTreeSuccessorIterator<T>(this.tree));
        }

        private static class BTreeSuccessorIterator<C extends Comparable<C>> implements java.util.Iterator<C> {

            private BTreeSuccessor<C> tree = null;
            private BTreeSuccessor.Node<C> lastNode = null;
            private C lastValue = null;
            private int index = 0;
            private Deque<BTreeSuccessor.Node<C>> toVisit = new ArrayDeque<BTreeSuccessor.Node<C>>();

            protected BTreeSuccessorIterator(BTreeSuccessor<C> tree) {
                this.tree = tree;
            ...
            public C next() {
                ...
                while (toVisit.size()>0) {
                    ...
                    BTreeSuccessor.Node<C> n = toVisit.pop();
